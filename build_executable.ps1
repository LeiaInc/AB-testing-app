...
# Helper function to rename executable with retry (handles antivirus delays)
function Rename-ExeWithRetry {
    param(
        [string]$SourceName,
        [string]$DestName,
        [int]$MaxRetries = 5,
    ...
    $sourcePath = "dist\$SourceName"
    $destPath = "dist\$DestName"
    
    for ($i = 1; $i -le $MaxRetries; $i++) {
        if (Test-Path $sourcePath) {
            try {
                Rename-Item $sourcePath $DestName -Force -ErrorAction Stop
                $fileSize = (Get-Item $destPath).Length / 1MB
                Write-Host "  OK $DestName ($([math]::Round($fileSize, 2)) MB)" -ForegroundColor Green
                return $true
            } catch {
                Write-Host "  Retry $i/$MaxRetries - waiting for file lock..." -ForegroundColor Yellow
                Start-Sleep -Milliseconds $RetryDelayMs
            }
        } else {
            if ($i -eq 1) {
                Write-Host "  Waiting for $SourceName..." -ForegroundColor Yellow
            }
            Start-Sleep -Milliseconds $RetryDelayMs
        }
    }
    ...
    # Final check
    if (Test-Path $sourcePath) {
        Rename-Item $sourcePath $DestName -Force
        $fileSize = (Get-Item $destPath).Length / 1MB
        Write-Host "  OK $DestName ($([math]::Round($fileSize, 2)) MB)" -ForegroundColor Green
        return $true
    }
    
    Write-Host "  X $SourceName not found! (likely removed by antivirus)" -ForegroundColor Red
    Write-Host "    Add 'E:\github\AB-testing-app\dist' to Windows Defender exclusions" -ForegroundColor Yellow
    return $false
}
...
if (-not (Rename-ExeWithRetry "AlgoSwitcher.exe" "AlgoSwitcher_$gitHash.exe")) {
    $success = $false
}

if (-not (Rename-ExeWithRetry "MultivariateTesting.exe" "MultivariateTesting_$gitHash.exe")) {
    $success = $false
}

if (-not $success) {
    Write-Host ""
    Write-Host "  Some executables were not found. To fix antivirus false positives:" -ForegroundColor Yellow
    Write-Host "  1. Open Windows Security > Virus & threat protection" -ForegroundColor Cyan
    Write-Host "  2. Manage settings > Add or remove exclusions" -ForegroundColor Cyan
    Write-Host "  3. Add folder: E:\github\AB-testing-app\dist" -ForegroundColor Cyan
    exit 1
}

$exePath1 = "dist\AlgoSwitcher_$gitHash.exe"
$exePath2 = "dist\MultivariateTesting_$gitHash.exe"
